...
Given("I am logged in as an admin 215036R_admin", async function () {
  const page = this.page;
  await ensureAdminLoggedIn(page);
});

Given("I am on the Dashboard page 215036R_admin", async function () {
  const page = this.page;
...
Then("I should see the Dashboard header 215036R_admin", async function () {
  const page = this.page;
  await expect(page.locator(".container-fluid h3")).toContainText("Dashboard");
});

Then("the Dashboard tab should be active 215036R_admin", async function () {
  const page = this.page;
...
Then("the sidebar navigation should be visible 215036R_admin", async function () {
  const page = this.page;
...
Then("all dashboard cards should be visible 215036R_admin", async function () {
  const page = this.page;
...
Then(
  "all dashboard cards should have non-empty summary data 215036R_admin",
  async function () {
    const page = this.page;
...
Then(
  "all dashboard cards should have visible action buttons 215036R_admin",
  async function () {
    const page = this.page;
    const cards = ["Categories", "Plants", "Sales"];
...
When("I click the {string} button 215036R_admin", async function (buttonText) {
  const page = this.page;
  const button = page.locator(`a:has-text("${buttonText}")`);
  await button.click();
});

When("I click the Categories sidebar navigation 215036R_admin", async function () {
  const page = this.page;
...
When("I click the Plants sidebar navigation 215036R_admin", async function () {
  const page = this.page;
...
When("I click the Sales sidebar navigation 215036R_admin", async function () {
  const page = this.page;
...
Then("I should be on the Categories page 215036R_admin", async function () {
  const page = this.page;
...
Then("I should be on the Plants page 215036R_admin", async function () {
  const page = this.page;
...
Then("I should be on the Sales page 215036R_admin", async function () {
  const page = this.page;
...
Then("the Categories tab should be active 215036R_admin", async function () {
  const page = this.page;
  const categoriesTab = page.locator('a.nav-link[href="/ui/categories"]');
  await expect(categoriesTab).toHaveClass(/active/);
});

Then("the Plants tab should be active 215036R_admin", async function () {
  const page = this.page;
  const plantsTab = page.locator('a.nav-link[href="/ui/plants"]');
  await expect(plantsTab).toHaveClass(/active/);
});

Then("the Sales tab should be active 215036R_admin", async function () {
  const page = this.page;
  const salesTab = page.locator('a.nav-link[href="/ui/sales"]');
