...
Given("I am logged in as a user 214079R_User", async function () {
  const page = this.page;
  await ensureUserLoggedIn(page);
});

Given("I am on the User Categories page 214079R_User", async function () {
  const page = this.page;
  await page.goto(`${BASE_URL}/ui/categories`);
  await page.waitForLoadState("networkidle");
});

Then("I should see the Categories header 214079R_User", async function () {
  const page = this.page;
...
Then("I should see the Search input 214079R_User", async function () {
  const page = this.page;
...
Then("I should see the Parent filter 214079R_User", async function () {
  const page = this.page;
...
Then(
  "I should see the Search and Reset buttons 214079R_User",
  async function () {
    const page = this.page;
...
Then("I should see the Categories table 214079R_User", async function () {
  const page = this.page;
...
Then(
  "I should see the Categories table headers 214079R_User",
  async function () {
    const page = this.page;
...
When("I search for a valid category name 214079R_User", async function () {
  const page = this.page;
  await page.waitForSelector("table tbody tr:first-child");
  const firstRowCategoryName = await page
    .locator("table tbody tr:first-child td:nth-child(2)")
    .textContent();

  this.searchedCategoryName = firstRowCategoryName.trim();
...
Then("I should see the category in the table 214079R_User", async function () {
  const page = this.page;
...
When(
  "I search for category {string} 214079R_User",
...
Then("I should see {string} text 214079R_User", async function (text) {
  const page = this.page;
  const emptyText = page.locator(`text=${text}`);
  await expect(emptyText).toBeVisible();
});

When("I filter by the first parent category 214079R_User", async function () {
  const page = this.page;
  const parentFilter = page.locator('select[name="parentId"]');
...
Then(
  "I should see categories with the selected parent 214079R_User",
  async function () {
    const page = this.page;
    const rows = page.locator("tbody tr");
    const count = await rows.count();
...
When("I click the Reset button 214079R_User", async function () {
  const page = this.page;
  const resetButton = page.locator("a:has-text('Reset')");
  await resetButton.click();
});

Then("I should see all categories again 214079R_User", async function () {
  const page = this.page;
  const rows = page.locator("tbody tr");
...
When(
  "I sort categories by ID {string} 214079R_User",
  async function (direction) {
    const page = this.page;
    const header = page.locator(`th a:has-text("ID")`);
    // Spec logic: Click once -> Asc, Click again -> Desc
    // The scenario order is Asc then Desc, so simplified "just click" works perfectly
    await header.click();
  },
);

Then(
  "the categories should be sorted by ID {string} 214079R_User",
  async function (direction) {
    const page = this.page;
    const header = page.locator(`th a:has-text("ID")`);
...
When(
  "I sort categories by {string} {string} 214079R_User",
  async function (column, direction) {
    ...
    // The scenario order is Desc then Asc, so simplified "just click" works perfectly.
    await header.click();
  },
);

Then(
  "the categories should be sorted by {string} {string} 214079R_User",
  async function (column, direction) {
    const page = this.page;
    const header = page.locator(`th a:has-text("${column}")`);
    const arrow = header.locator("span");
